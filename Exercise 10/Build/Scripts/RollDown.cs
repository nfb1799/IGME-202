...
public class RollDown : MonoBehaviour
{
    ...
    void OnGUI()
    {
        if (vel.magnitude < 0.25 || transform.position.x < -99 || transform.position.x > 99 || transform.position.z < -99 || transform.position.z > 99)
            GUI.Label(new Rect(10, 50, 500, 100), "RoBall\n<b>X=</b> " + transform.position.x + " <b>Y=</b> " + transform.position.y + " <b>Z=</b> " + transform.position.z);
    }
}
