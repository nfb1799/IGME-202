...
public class CollisionDetector: MonoBehaviour 
{
	...
	{
		float aRad = a.GetComponent<SpriteInfo>().radius;
		float bRad = b.GetComponent<SpriteInfo>().radius;
		float min = aRad + bRad;
		float dist = Mathf.Pow((a.transform.position.x - b.transform.position.x), 2) + Mathf.Pow((a.transform.position.y - b.transform.position.y), 2);

		if (dist < Mathf.Pow(min, 2))
			return false;
		return true;
